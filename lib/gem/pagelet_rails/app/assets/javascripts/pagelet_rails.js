...
(function (w) {
  ...
  root.append_script_tag = function (text) {
  ...
  root.load_directly = function(data) {
    $.ajax({
      url: data.url,
      dataType: 'script',
      headers: {
      ...
      var html = JST['views/pagelet_load_failed']({
        ...
        reload_function: "PageletRails.load_pagelets('#" + data.id + "');"
      });
      data.elem.html(html)
    });
  };

  root.load_through_batch_proxy = function(urls) {
  ...
  root.load_pagelets = function(selector) {
    ...
    for (var group_name in groups) {
      if (groups.hasOwnProperty(group_name)) {
        ...
        if (group.length == 1) {
          root.load_directly(group[0]);
        } else {
          var urls = group.map(function(e) { return e.url; });
          root.load_through_batch_proxy(urls);
        }
      }
    }
  };

  root.pagelet_arrived = function(id, content) {
    root.place_into_container(id, content);
    $(document).trigger('pagelet-loaded');
  };

  root.place_into_container = function(id, content) {
    $('#' + id).html(content);
  };

  root.process_data_remote_tags = function() {
